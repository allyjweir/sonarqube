...
public class ByteCharsetDetector {
  ...
  public Charset detect(byte[] buf) {
    ...
    charsets.add(userConfiguration);
    ...
    if (utf8Result.valid() == Validation.YES) {
      ...
      return detectAscii(buf, charsets);
    }
  ...
  private Charset detectAscii(byte[] buf, Set<Charset> charsets) {
    for (Charset c : charsets) {
      if (!isUtf16Or32(c) && validator.tryDecode(buf, c)) {
        return c;
      }
    }
