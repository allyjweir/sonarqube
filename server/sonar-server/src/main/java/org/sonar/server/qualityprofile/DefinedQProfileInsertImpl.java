...
public class DefinedQProfileInsertImpl implements DefinedQProfileInsert {
  ...
  public void create(DbSession session, DefinedQProfile definedQProfile, OrganizationDto organization, List<ActiveRuleChange> changes) {
    ...
    definedQProfile.getActiveRules()
      .stream()
  ...
  private ActiveRuleChange insertActiveRule(DbSession session, QualityProfileDto profileDto, org.sonar.api.rules.ActiveRule activeRule, long now) {
    ...
    insertActiveRuleParams(session, activeRule, ruleKey, dto);

    return ActiveRuleChange.createFor(ActiveRuleChange.Type.ACTIVATED, ActiveRuleKey.of(profileDto.getKey(), ruleKey));
  }

  private void insertActiveRuleParams(DbSession session, org.sonar.api.rules.ActiveRule activeRule, RuleKey ruleKey, ActiveRuleDto dto) {
    ...
    ruleRepository.getRuleParams(ruleKey)
      .forEach(param -> {
        String activeRuleValue = valuesByParamKey.get(param.getName());
        insertParam(session, dto, param, activeRuleValue == null ? param.getDefaultValue() : activeRuleValue);
      });
  }

  private void insertParam(DbSession session, ActiveRuleDto dto, RuleParamDto param, @Nullable String value) {
    if (value != null) {
      ActiveRuleParamDto paramDto = ActiveRuleParamDto.createFor(param);
      paramDto.setValue(validateParam(param, value));
      dbClient.activeRuleDao().insertParam(session, dto, paramDto);
    }
  }
